...
void DiscQueue::insertProcess(PCBProcess myProcess, std::string refClass) {
...
void DiscQueue::printProcessNotIsRQ() {
  if(queue == 0)
    cout << "Empty DiscQueue\n";
  else {
    Queue *temp;
    temp = queue;
    int counterLines=0;
...
PCBProcess DiscQueue::getProcess(std::string refClass) {
...
bool DiscQueue::findProcess(std::string refClass) {
  if(queue == 0)
    return false;
  else {
    std::string nameProc;
