...
void PrinterQueue::insertProcess(PCBProcess myProcess, std::string refClass) {
...
void PrinterQueue::printProcessNotIsRQ() {
  if(queue == 0)
    cout << "Empty PrintQueue\n";
  else {
    Queue *temp;
    temp = queue;
    int counterLines=0;
...
PCBProcess PrinterQueue::getProcess(std::string refClass) {
...
//Busca un proceso por su nombre en PrinterQueue
bool PrinterQueue::findProcess(std::string refClass) {
  if(queue == 0)
    return false;
  else {
    std::string nameProc;
